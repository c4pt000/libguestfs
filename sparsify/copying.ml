        ...
        if List.mem fs zeroes then (
          ...
          if mounted then (
            if is_readonly_btrfs_snapshot fs "/" then (
              ...
              message (f_"Fill free space in image %s with zeros") fs;
              g#zero_free_space "/"
            )
          ) else (
            ...
            if is_linux_x86_swap then (
              message (f_"Clearing Linux %s swap on swap") fs;
  ...
  List.iter (
    fun vg ->
      if not (List.mem vg ignores) then (
        ...
        if created then (
          message (f_"Fill free space in volgroup image %s with zeros") vg;
